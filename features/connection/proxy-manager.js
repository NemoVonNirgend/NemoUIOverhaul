...
function getAll() {
    ...
    if (!Array.isArray(extension_settings.NemoUIOverhaul.customProxies)) {
        extension_settings.NemoUIOverhaul.customProxies = [];
    }
    return extension_settings.NemoUIOverhaul.customProxies;
}
...
function save(proxies) {
    ...
    extension_settings.NemoUIOverhaul.customProxies = proxies;
...
function loadCSS() {
    if (!document.querySelector('link[href*="proxy-manager.css"]')) {
        ...
        link.rel = 'stylesheet';
        link.href = 'scripts/extensions/third-party/NemoUIOverhaul/features/connection/proxy-manager.css';
